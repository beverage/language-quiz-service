...
@pytest.mark.asyncio
async def test_correctness_prompt(prompt_generator: SentencePromptGenerator, mock_sentence: MagicMock):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, False, False, True, ["__sentence_properties", "__compliment_object_direct"]),
        (False, True, False, True, ["__sentence_properties", "__compliment_object_indirect"]),
        (False, False, True, True, ["__sentence_properties", "__negation"]),
        (False, False, False, False, ["__sentence_properties", "__correctness"]),
        (True, True, True, False, ["__sentence_properties", "__compliment_object_direct", "__compliment_object_indirect", "__negation", "__correctness"]),
    ]
)
async def test_generate_sentence_prompt_logic(
    ...
    mock_sentence.direct_object = DirectObject.MASCULINE if has_cod else DirectObject.NONE
    mock_sentence.indirect_pronoun = IndirectPronoun.PLURAL if has_coi else IndirectPronoun.NONE
    mock_sentence.negation = Negation.PAS if has_negation else Negation.NONE
    mock_sentence.is_correct = is_correct

    mock_verb = MagicMock()
    
    with patch.object(prompt_generator, '_SentencePromptGenerator__sentence_properties', return_value="") as mock_sp, \
         patch.object(prompt_generator, '_SentencePromptGenerator__compliment_object_direct', return_value="") as mock_cod, \
         patch.object(prompt_generator, '_SentencePromptGenerator__compliment_object_indirect', return_value="") as mock_coi, \
         patch.object(prompt_generator, '_SentencePromptGenerator__negation', return_value="") as mock_neg, \
         patch.object(prompt_generator, '_SentencePromptGenerator__correctness', return_value="") as mock_corr:

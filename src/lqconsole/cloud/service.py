...
async def check_cluster_and_service_exists(ecs, cluster_name=None, service_name=None):
    ...
    """
    cluster_response = await asyncio.get_running_loop().run_in_executor(None, partial(
    ...
    service_response = await asyncio.get_running_loop().run_in_executor(None, partial(
    ...
    if not service_response['services']:
        log.error(f"Service {service_display_name} does not exist in cluster {cluster_display_name}.")
        return False
...
async def wait_for_service_status(ecs, cluster_name, service_name, task_count):
    ...
    while True:
        service_status = await asyncio.get_running_loop().run_in_executor(None, partial(
            ecs.describe_services,
            cluster=cluster_name,
            services=[service_name]
        ))

        if service_status['services'][0]['desiredCount'] == task_count and service_status['services'][0]['runningCount'] == task_count:
            print('')
            break
...
async def down():
    ...
    if not await check_cluster_and_service_exists(ecs, cluster_name=cluster_name, service_name=service_name):
        return

    cluster_display_name = __cluster_display_name(cluster_name)
    service_display_name = __service_display_name(service_name)

    down_response = await asyncio.get_running_loop().run_in_executor(None, partial(
        ecs.update_service,
        cluster=cluster_name,
    ...
    log.info(f"Service {service_display_name} in cluster {cluster_display_name} is now set to 0 desired count. All tasks will be {Style.BOLD}{Color.STRONG_RED}stopped{Style.RESET}.")

    await wait_for_service_status(ecs, cluster_name, service_name, 0)

    log.info(f"Service {service_display_name} is now fully {Style.BOLD}{Color.STRONG_RED}stopped{Style.RESET}.")
...
async def up(count: int = 1):
    ...
    up_response = await asyncio.get_running_loop().run_in_executor(None, partial(
        ecs.update_service,
        cluster=cluster_name,
        service=service_name,
        desiredCount=count
    ...
    log.info(f"Service {service_display_name} in cluster {cluster_display_name} is now set with desired task count of {count}. Task will be {Style.BOLD}{Color.STRONG_GREEN}started{Style.RESET}.")

    await wait_for_service_status(ecs, cluster_name, service_name, count)

    log.info(f"Service {service_display_name} is now {Style.BOLD}{Color.STRONG_GREEN}running{Style.RESET} with {count} task(s).")
    return up_response
